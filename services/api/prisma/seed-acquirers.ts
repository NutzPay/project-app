...
async function seedAcquirers() {
  ...
  try {
    // DigitoPay (PIX)
    const digitoPayConfig = {
      baseUrl: process.env.DIGITOPAY_BASE_URL || 'https://app.digitopayoficial.com.br',
      accountId: process.env.DIGITOPAY_ACCOUNT_ID || '',
      clientId: process.env.DIGITOPAY_CLIENT_ID || '',
      secret: process.env.DIGITOPAY_SECRET || ''
    };

    await prisma.paymentAcquirer.upsert({
      where: { slug: 'digitopay' },
      update: {
        apiConfig: JSON.stringify(digitoPayConfig),
        status: digitoPayConfig.accountId ? 'ACTIVE' : 'INACTIVE'
      },
      create: {
        name: 'DigitoPay',
        slug: 'digitopay',
        ...
        feeConfig: JSON.stringify({
          deposit: { type: 'PERCENTAGE', value: 2.5 },
          withdrawal: { type: 'PERCENTAGE', value: 1.5 }
        }),
        testMode: true,
        supportsDeposits: true,
        supportsWithdrawals: false,
        supportsWebhooks: true,
        description: 'Gateway PIX brasileiro com processamento em tempo real',
        logoUrl: null,
        documentationUrl: 'https://digitopay.com.br/docs'
      }
    });

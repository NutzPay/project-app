...
export default function DashboardPage() {
  ...
  // Função para reformatar descrições de transações
  const formatTransactionDescription = (description: string) => {
    // Handle new format "Compra de USDT: R$ X → Y USDT"
    if (description?.includes('Compra de USDT:')) {
      const match = description.match(/Compra de USDT: R\$ ([\d.]+) → ([\d.]+) USDT/);
      if (match) {
        const brlValue = parseFloat(match[1]);
        const usdtValue = parseFloat(match[2]);
        return `${formatBRL(brlValue)} → ${formatCrypto(usdtValue, 'USDT', { maximumFractionDigits: 2 })}`;
      }
    }
    // Handle legacy format "XGate payment: R$ X → Y USDT"

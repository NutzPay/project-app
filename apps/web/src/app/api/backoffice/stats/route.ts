...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // Total de usuários
      prisma.user.count(),
      // Stats reais de transações USDT (filtrado por período)
      prisma.uSDTTransaction.groupBy({
        ...
        deposits: data.filter(s => s.type === 'DEPOSIT').reduce((sum, item) => sum + item._count, 0),
        withdrawals: data.filter(s => s.type === 'WITHDRAWAL').reduce((sum, item) => sum + item._count, 0),
        depositVolume: data.filter(s => s.type === 'DEPOSIT' && s.status === 'COMPLETED').reduce((sum, item) => sum + Number(item._sum?.amount || 0), 0),

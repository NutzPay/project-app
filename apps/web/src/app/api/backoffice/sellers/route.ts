...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const stats = {
      ...
      activeSellers: statsData.find(s => s.status === 'ACTIVE')?._count || 0,
      pendingSellers: statsData.find(s => s.status === 'PENDING')?._count || 0,
      suspendedSellers: statsData.find(s => s.status === 'SUSPENDED')?._count || 0,
    };
    ...
    const paginatedSellers = users.map(user => ({
      ...
      dealSummary: {
        totalVolume: 0, // TODO: Calculate from actual deals
        activeDeals: user._count.createdOffers,
        lastDealDate: undefined, // TODO: Get from actual deals
      },
      usdtWallet: user.usdtWallet ? {
        balance: parseFloat(user.usdtWallet.balance.toString()),
        totalTransacted: 0, // TODO: Calculate from transactions
      } : undefined,
    }));

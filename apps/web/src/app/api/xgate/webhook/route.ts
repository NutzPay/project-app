...
async function handlePaidStatus(payload: XGateWebhookPayload): Promise<void> {
  ...
  try {
    ...
    // Interpretar o amount baseado no campo name/operation
    let usdtAmount: number;
    let brlAmount: number;
    
    if (payload.name === 'USDT' || payload.cryptocurrency === 'USDT') {
      // Amount é em USDT
      usdtAmount = payload.amount;
      // Estimar BRL baseado na cotação padrão (fallback se API falhar)
      try {
        const currentRate = await priceCalculator.getUSDTPrice();
        brlAmount = usdtAmount * currentRate * 1.1; // Inclui taxa estimada
      } catch (error) {
        console.warn('⚠️ Failed to get USDT price, using fallback rate');
        const fallbackRate = 5.8; // BRL per USDT fallback
        brlAmount = usdtAmount * fallbackRate;
      }
    } else {
      // Amount é em BRL (formato antigo)  
      brlAmount = payload.amount;
      const exchangeRate = 0.17; // BRL to USDT rate
      usdtAmount = brlAmount * exchangeRate;
    }
    
    console.log('💰 Interpreted amounts:', {
      usdtAmount: `${usdtAmount} USDT`,
      brlAmount: `R$ ${brlAmount}`,
      interpretation: payload.name === 'USDT' ? 'Amount is USDT' : 'Amount is BRL'
    });
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.uSDTTransaction.update({
        ...
        data: {
          ...
          description: `XGate payment: ${formatBRL(brlAmount)} → ${formatCrypto(usdtAmount, 'USDT', { maximumFractionDigits: 2 })}`
        }
      });

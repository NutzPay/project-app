...
import { useUSDTPrice } from '@/hooks/useCryptoPrice';
import { useUSDTData } from '@/hooks/useUSDTData';

export default function USDTQuote() {
  const [showChart, setShowChart] = useState(false);
  // Use real CoinMarketCap data for USDT with fallback to mock data
  const { 
    data: realUsdtData, 
  ...
  // Fallback to mock data hook if real data fails
  const { data: mockUsdtData, historicalData: mockHistoricalData, loading: mockLoading, error: mockError, refetch: mockRefetch } = useUSDTPrice({
    vsCurrency: 'brl',
    updateInterval: 300000, // 5 minutes
    enableHistorical: true
  });
  ...
  const usdtData = realUsdtData || mockUsdtData;
  const historicalData = realHistoricalData.length > 0 ? realHistoricalData : mockHistoricalData;
  const loading = realLoading || mockLoading;
  const error = realError || mockError;
  const refetch = () => {
    realRefetch();
    mockRefetch();
  };
  ...
  const isPositive = realUsdtData ? realIsPositive : (usdtData?.changePercent24h >= 0);
  const minPrice = historicalData.length > 0 ? Math.min(...historicalData.map(d => d.price)) : 0;
  const maxPrice = historicalData.length > 0 ? Math.max(...historicalData.map(d => d.price)) : 0;
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center space-x-2 mt-1">
                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                  ...
                  <svg className={`w-3 h-3 mr-1 ${isPositive ? 'rotate-0' : 'rotate-180'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className="text-xs text-gray-500">
                  {isPositive ? '+' : ''}{formatBRL(usdtData.change24h, { showSymbol: false })} (24h)
                </span>
              </div>
            </div>
            ...
            <div className="flex space-x-2">
              ...
              <button
                onClick={refetch}
                className="p-2 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors text-sm"
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            ...
            <div className="flex items-center space-x-2">
              ...
              {(realError || error) && (
                <span className="text-red-500" title={realError || error}>
                  ⚠️
                </span>
              )}
            </div>

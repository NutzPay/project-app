...
export default function USDTInvestmentCard({ 
  ...
  // Dados históricos estáveis (sem random)
  const getHistoricalData = () => {
    const monthsMap = { '6M': 6, '1Y': 12, '2Y': 24 };
    const months = monthsMap[selectedPeriod];
    const basePrice = usdtData.currentPrice;
    
    // Padrão de preços fixo baseado no índice para estabilidade
    const pricePattern = [0.96, 0.98, 1.02, 1.01, 0.99, 1.03, 1.02, 0.97, 1.01, 0.98, 1.04, 1.00];
    const data = [];
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      // Usa padrão fixo para preços estáveis
      const patternIndex = i % pricePattern.length;
      const priceMultiplier = pricePattern[patternIndex];
  ...
  useEffect(() => {
    ...
    // Atualizar preço USDT a cada 30 segundos
  ...
  const loadUSDTPrice = async () => {
    try {
      ...
      if (result.success && result.data) {
        console.log('💰 Setting USDT price:', result.data.currentPrice);
        setUsdtPriceData(result.data);
      } else {
